/// DBC file: _can_dbc/243.dbc    Self node: MOTOR
...
extern const DRIVER_TX_MOTOR_CMD_t                            MOTOR_CMD__MIA_MSG;

/// Not generating code for DRIVER_TX_HEARTBEAT_encode() since the sender is DRIVER and we are MOTOR

/// Not generating code for SENSOR_TX_sensorback_encode() since the sender is SENSOR and we are MOTOR

/// Not generating code for SENSOR_TX_SONARS_encode() since the sender is SENSOR and we are MOTOR

/// Not generating code for DRIVER_TX_MOTOR_CMD_encode() since the sender is DRIVER and we are MOTOR
...
/// @param hdr  The header of the message to validate its DLC and MID; this can be NULL to skip this check
static inline bool DRIVER_TX_MOTOR_CMD_decode(DRIVER_TX_MOTOR_CMD_t *to, const uint64_t *from, const msg_hdr_t *hdr)
{
    const bool success = true;
    if (NULL != hdr && (hdr->dlc != DRIVER_TX_MOTOR_CMD_HDR.dlc || hdr->mid != DRIVER_TX_MOTOR_CMD_HDR.mid)) {
        return !success;
    }
    uint64_t raw_signal;
    const uint8_t *bytes = (const uint8_t*) from;
    ...
    raw_signal = 0;
    raw_signal |= ((uint64_t)((bytes[0] >> 0) & 0xff)) << 0; ///< 8 bit(s) from B0
    to->MOTOR_CMD_steer = (raw_signal * 1.0) + (0);
    ...
    raw_signal = 0;
    raw_signal |= ((uint64_t)((bytes[2] >> 0) & 0xff)) << 0; ///< 8 bit(s) from B16
    to->MOTOR_CMD_angle = (raw_signal * 1.0) + (0);
...
/// @returns true if the MIA just occurred
/// @post    If the MIA counter is not reset, and goes beyond the MIA value, the MIA flag is set
static inline bool DRIVER_TX_MOTOR_CMD_handle_mia(DRIVER_TX_MOTOR_CMD_t *msg, uint32_t time_incr_ms)
{
    bool mia_occurred = false;
#if 0
    const mia_info_t old_mia = msg->mia_info;
